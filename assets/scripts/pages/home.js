...
function attribute(string) {
  if (!container.hasAttribute(string)) {
    return false;
  }
...
async function fetchTricks(qty = 3) {
  const data = await fetch(
    `${url}${endpoint}?max=${qty}&index=${index}&total=${total}`
  );
...
function displayTrick(trick) {
  ...
  div.innerHTML = `
    <div class="card trick-card h-100">
        ...
        <div class="trick-card-infos">
            <a class="trick-card-title" href="${url}tricks/details/${
    trick.t_slug
  }">${trick.t_name}</a>
            ${loggedIn ? actions : ""}
        </div>
  ...
  const button = div.querySelector(".trick-card-delete");
  if (button) {
