...
const initialVideos = JSON.parse(tricksVideoElement.getAttribute("data-initial-values"));
...
const tricksVideoScrollable = document.getElementById('tricks-videos-scrollable');
...
const updateFrame = (id, value, iframe, i = index) => {
    ...
    if (updateIndex("add")) {
        ...
        radiosElements.forEach(radio => {
            radio.addEventListener("click", (event) => {
                ...
                updateFrame(input.value, radioElement.value, iframe, input.getAttribute("data-index"));
            });
        })
        input.addEventListener("input", (event) => {
            const value = div.querySelector("input[type='radio']:checked");
            updateFrame(event.target.value, value.value, iframe, event.target.getAttribute("data-index"));
        });
